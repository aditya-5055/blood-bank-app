const donationCompletedTemplate = (donorName, facilityName, bloodGroup, units, donatedTo) => {
  return `
  <!DOCTYPE html>
  <html>
  <head>
    ...
    <style>
      ...
      .hero-text { text-align: center; font-size: 42px; margin: 20px 0; }
  ...
  <body>
    <div class="container">
      ...
      <div class="body">
        ...
        <p>Your blood donation has been successfully recorded. You are a hero!</p>
        <div class="hero-text">🦸</div>
        <div class="details-box">
          <p><span>Donated To:</span> ${facilityName}</p>
          <p><span>Type:</span> ${donatedTo === "blood-bank" ? "Blood Bank" : "Hospital"}</p>
          <p><span>Blood Group:</span> ${bloodGroup}</p>
          <p><span>Units Donated:</span> ${units}</p>
          <p><span>Date:</span> ${new Date().toDateString()}</p>
        </div>
        <p>You are now eligible to donate again after <strong>90 days</strong>.</p>
        <p>Your donation can save up to <strong>3 lives</strong>. Thank you! ❤️</p>
      </div>
      <div class="footer">
...
module.exports = donationCompletedTemplate;